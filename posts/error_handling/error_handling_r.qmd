...
# description: 'The typical errors of a programming workflow are seen and the best practive to avoid errors and build robust scritps is shown on the example of R programming.'
...
Here, the error is in the code itself, but in what it does. Meaning that while the code will execute and produce an output, it will not be the desired one for us. In this case, the coder made a error of operator, putting a single `*` instead of `**` for the exponentiation.
...
In R, there is already implemented a mechanism that gives the programmer access to that logs from the low level code as to why an error occurred. Errors are returned as *objects* of class `try-error` and can be handled through the `tryCatch` function. It will attempt to run an expression provided as argument, and will catch the error objects in case of failures and provide access to the internals of the error. Let's look at it on the example of the previously defined function
...
However, some errors might still arrise:
